...
> ポイント（シーズン／通算）は専用テーブルを持たず、予想×結果から都度算出する。
...
| 0001_big_wrecking_crew      | （旧）`token_awards` 追加、`users` に `solana_address` / `wallet_verified_at` 追加 ※0003で撤去 |
| 0002_minor_mikhail_rasputin | `artists`・`songs` に外部DB由来カラム（source / external_id / image_url）と UNIQUE 追加    |
| 0003_special_kitty_pryde    | 記念トークン廃止に伴い `token_awards` を DROP、`users` の `solana_address` / `wallet_verified_at` を削除 |
